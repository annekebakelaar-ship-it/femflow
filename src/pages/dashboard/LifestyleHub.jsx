...
function ActiviteitDetail({ activiteit, faseInfo, onTerug, onStartOefening }) {
  ...
  return (
    <div style={{ padding: '0 16px 32px' }}>
      ...
      <p style={{ fontSize: 12, letterSpacing: '0.18em', color: '#A8998A', margin: '0 0 6px', fontFamily: sans, textTransform: 'uppercase' }}>{a.pijlerNaam}</p>
      <h1 style={{ fontFamily: serif, fontSize: 28, color: '#F5F2EB', margin: '0 0 8px', lineHeight: 1.2 }}>{a.titel}</h1>
      <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#DFB88A', margin: '0 0 16px', fontFamily: sans }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
        {a.fases.map(f => {
          ...
          return (
            <span key={f} style={{ fontSize: 12, padding: '4px 10px', borderRadius: 999, fontFamily: sans, background: nu ? 'rgba(212,163,115,0.3)' : 'rgba(212,163,115,0.1)', color: nu ? '#F5F2EB' : '#D4A373' }}>
              {FASE_POETIC[f]}{nu ? ' · nu' : ''}
            </span>
          )
        })}
      ...
      {OEFENINGEN[a.id] && (
        <button onClick={() => onStartOefening(OEFENINGEN[a.id])} style={{ width: '100%', padding: 15, marginBottom: 16, borderRadius: 16, border: 'none', cursor: 'pointer', fontFamily: sans, fontSize: 15, fontWeight: 600, color: '#211C1A', background: 'linear-gradient(135deg, #DFB88A, #D4A373)', boxShadow: '0 6px 24px rgba(212,163,115,0.3)' }}>
          Start de begeleide oefening
        </button>
      )}

      <div style={{ ...KAART, padding: 18, marginBottom: 16 }}>
        <p style={{ fontSize: 12, letterSpacing: '0.14em', color: '#A8998A', margin: '0 0 8px', fontFamily: sans }}>WAAROM DIT WERKT</p>
        <p style={{ fontSize: 14, lineHeight: 1.65, color: '#F5F2EB', margin: 0, fontFamily: sans }}>{a.waarom}</p>
      </div>

      {a.uitleg.map((par, i) => (
        <p key={i} style={{ fontSize: 14, lineHeight: 1.7, color: '#D9CFC2', margin: '0 0 14px', fontFamily: sans }}>{par}</p>
      ))}

      <div style={{ ...KAART, padding: 18, margin: '6px 0 16px' }}>
        <p style={{ fontSize: 12, letterSpacing: '0.14em', color: '#A8998A', margin: '0 0 10px', fontFamily: sans }}>ZO BEGIN JE</p>
        {a.stappen.map((s, i) => (
          <div key={i} style={{ display: 'flex', gap: 10, marginBottom: i < a.stappen.length - 1 ? 10 : 0 }}>
            <span style={{ fontFamily: serif, color: '#D4A373', fontSize: 14, lineHeight: 1.5, flexShrink: 0 }}>{i + 1}.</span>
            <span style={{ fontSize: 13, lineHeight: 1.55, color: '#F5F2EB', fontFamily: sans }}>{s}</span>
          </div>
        ))}
      </div>

      <div style={{ display: 'flex', gap: 10, padding: 16, borderRadius: 16, background: 'rgba(212,163,115,0.08)' }}>
        <Info size={15} color="#DFB88A" style={{ flexShrink: 0, marginTop: 2 }} />
        <p style={{ fontSize: 13, lineHeight: 1.6, color: '#D9CFC2', margin: 0, fontFamily: sans }}>{a.nuance}</p>
      </div>
    </div>
  )
}
...
function PTKaart() {
  ...
  return (
    <div style={{ ...KAART, margin: '12px 16px 0', padding: 18, border: '1px solid rgba(212,163,115,0.2)' }}>
      <p style={{ fontSize: 12, letterSpacing: '0.14em', color: '#DFB88A', margin: '0 0 6px', fontFamily: sans }}>PERSOONLIJKE BEGELEIDING</p>
      <p style={{ fontFamily: serif, fontSize: 17, color: '#F5F2EB', margin: '0 0 6px' }}>Liever iemand die meekijkt?</p>
      <p style={{ fontSize: 13, lineHeight: 1.6, color: '#A8998A', margin: '0 0 14px', fontFamily: sans }}>
      ...
      <button onClick={vraagAan} style={{ padding: '12px 22px', borderRadius: 999, border: 'none', cursor: 'pointer', fontFamily: sans, fontSize: 13, fontWeight: 600, color: '#211C1A', background: 'linear-gradient(135deg, #DFB88A, #D4A373)' }}>
        Vraag via WhatsApp
      </button>
    </div>
  )
          ...
          {open ? (
            ...
            <div style={{ paddingBottom: 24 }}>
              ...
              <div onClick={() => setOpenId(advies.activiteitId)} style={{ ...KAART, margin: '16px 16px 8px', padding: 18, cursor: 'pointer', border: '1px solid rgba(212,163,115,0.25)' }}>
                ...
                <p style={{ fontFamily: serif, fontSize: 20, color: '#F5F2EB', margin: '0 0 6px' }}>{advies.kop}</p>
                <p style={{ fontSize: 13, lineHeight: 1.6, color: '#D9CFC2', margin: '0 0 10px', fontFamily: sans }}>{advies.reden}</p>
                {adviesAct && (
                  <p style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#D4A373', margin: 0, fontFamily: sans }}>
