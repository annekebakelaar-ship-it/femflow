export default function TermsOfService() {
  return (
    <div style={{
      maxWidth: '800px',
      margin: '0 auto',
      padding: 'var(--space-lg)',
      fontFamily: 'var(--font-sans)',
      lineHeight: '1.7',
      color: 'var(--ink)',
    }}>
      <h1 style={{ fontSize: '26px', fontFamily: 'var(--font-display)', fontWeight: '500', lineHeight: 1.25, marginBottom: 'var(--space-lg)' }}>
        Gebruikersvoorwaarden Ovari
      </h1>

      <p style={{ fontSize: '13px', fontFamily: 'var(--font-sans)', fontWeight: '400', color: 'var(--ink-3)', marginBottom: 'var(--space-lg)' }}>
        Laatst bijgewerkt: 4 juni 2026
      </p>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontFamily: 'var(--font-display)', fontWeight: '500', lineHeight: 1.25, marginBottom: 'var(--space-md)' }}>1. Acceptatie van Voorwaarden</h2>
        <p>
          Door Ovari te gebruiken, gaat u ermee akkoord dat u deze Gebruikersvoorwaarden, ons Privacybeleid en alle toepasselijke wetten hebt gelezen en aanvaard.
        </p>
        <p>
          Als u niet akkoord bent, mag u de app niet gebruiken.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>2. Dienstbeschrijving</h2>

        <p>
          Ovari is een gezondheids- en welzijnsapp van YouCaps, gericht op:
        </p>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>Menstruatiecyclus volgen en analyseren</li>
          <li>Symptomen en gezondheidsgegevens registreren</li>
          <li>Dagelijkse readiness-scores berekenen (op basis van slaap, HRV, hartslag en cyclus)</li>
          <li>Gepersonaliseerde gezondheidsadviezen en artikelen aanbevelen</li>
          <li>Optioneel: wearable-gegevens synchroniseren (bijvoorbeeld Oura)</li>
          <li>Optioneel: Supplementenabonnementen beheren</li>
        </ul>

        <p>
          <strong>Belangrijk:</strong> Ovari is <strong>geen medische dienst</strong>.
          Raadpleeg altijd uw huisarts voor medisch advies, diagnoses of behandeling.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>3. Accountvereisten</h2>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          3.1 Wie kan een account aanmaken?
        </h3>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>U moet minstens 18 jaar oud zijn</li>
          <li>U moet een geldig e-mailadres hebben</li>
          <li>U bent verantwoordelijk voor uw inloggegevens (inlogcode per e-mail of uw Google-account)</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          3.2 Accountbeveiliging
        </h3>
        <p>
          U bent verantwoordelijk voor:
        </p>
        <ul>
          <li>Uw e-mailadres veilig houden (het is uw login)</li>
          <li>Inlogcodes niet delen met anderen</li>
          <li>Uw Google-account beveiligen als u daarmee inlogt</li>
          <li>Uw browser-sessie veilig houden</li>
          <li>Ongeautoriseerde toegang onmiddellijk rapporteren</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          We zijn niet aansprakelijk voor ongeautoriseerde toegang die aan u te wijten is.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>4. Aanvaardbaar Gebruik</h2>

        <p>
          U mag de app <strong>NIET</strong> gebruiken voor:
        </p>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>Illegale activiteiten of regelschendingen</li>
          <li>Hacken, phishing of cyberaanvallen</li>
          <li>Andermans accounts of gegevens aanvallen</li>
          <li>Spammen, virussen of malware</li>
          <li>Onwettige opslag of verspreiding van andermans gezondheidsgegevens</li>
          <li>Misleidende medische informatie verspreiden</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          De app is niet geschikt voor acute medische of mentale crisissituaties.
          Neem in die gevallen contact op met uw huisarts, de huisartsenpost of bel 112.
        </p>

        <p style={{ marginTop: 'var(--space-md)' }}>
          Schendingen zullen resulteren in accountbeëindiging.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>5. Gezondheidsgegevens & Zelfverantwoordelijkheid</h2>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          5.1 Geen Medisch Advies
        </h3>
        <p>
          <strong>Ovari verleent GEEN medisch advies.</strong> De app is:
        </p>
        <ul>
          <li>Een trackinghulpmiddel (voor educatie en zelfkennis)</li>
          <li>Niet bedoeld ter vervanging van professionele medische adviezen</li>
          <li>Niet geverifieerd door medische autoriteiten</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          <strong>Raadpleeg een arts voor:</strong>
        </p>
        <ul>
          <li>Medische diagnoses</li>
          <li>Behandelingsadviezen</li>
          <li>Hormonale vragen of medicijnvragen</li>
          <li>Acute medische nood (bel 112)</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          5.2 Nauwkeurigheid van Gegevens
        </h3>
        <p>
          U bent verantwoordelijk voor:
        </p>
        <ul>
          <li>De juistheid van uw gezondheidsgegevens</li>
          <li>Het niet invoeren van medische informatie van anderen</li>
          <li>Het begrijpen dat handmatig ingevoerde gegevens minder nauwkeurig zijn dan wearables</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          5.3 Wearable-gegevens
        </h3>
        <p>
          Als u een wearable koppelt (bijvoorbeeld een Oura-ring):
        </p>
        <ul>
          <li>De voorwaarden van de wearable-aanbieder zijn ook van toepassing</li>
          <li>YouCaps is niet verantwoordelijk voor gegevensverlies of fouten bij de wearable-aanbieder</li>
          <li>U geeft YouCaps toestemming om uw wearable-gegevens te benaderen en op te slaan</li>
        </ul>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>6. Intellectueel Eigendom</h2>

        <p>
          <strong>Wat eigendom van YouCaps is:</strong>
        </p>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>App-code, ontwerp, algoritmen</li>
          <li>Merknamen, logo's, handelsnamen</li>
          <li>Artikelinhoud in de kennisbank</li>
          <li>De berekende readiness-scores en onderliggende rekenmodellen</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          <strong>Wat uw eigendom blijft:</strong>
        </p>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>Uw persoonlijke gezondheidsgegevens</li>
          <li>Uw symptoomnotities en opmerkingen</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          U mag onze app-inhoud niet kopiëren of reproduceren zonder toestemming (behalve persoonlijk gebruik).
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>7. Beperking van Aansprakelijkheid</h2>

        <p style={{ background: 'var(--surface-warm)', padding: 'var(--space-md)', borderRadius: '8px', marginBottom: 'var(--space-md)' }}>
          <strong>⚠️ BELANGRIJKE JURIDISCHE WAARSCHUWING:</strong>
        </p>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          7.1 De app wordt geleverd zoals hij is ("as-is")
        </h3>
        <p>
          Ovari wordt aangeboden op "as-is"-basis. We garanderen <strong>NIET</strong>:
        </p>
        <ul>
          <li>Voortdurende beschikbaarheid (onderhoudsvensters kunnen nodig zijn)</li>
          <li>Foutloze werking (bugs kunnen voorkomen)</li>
          <li>Nauwkeurigheid van berekeningen (readiness scores zijn schattingen)</li>
          <li>Compatibiliteit met alle apparaten</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          7.2 Geen Aansprakelijkheid Voor:
        </h3>
        <ul style={{ marginBottom: 'var(--space-md)' }}>
          <li>Gezondheidsresultaten of medische complicaties</li>
          <li>Financieel verlies of zakelijke schade</li>
          <li>Gegevensverlies (ondanks onze beveiligingsinspanningen)</li>
          <li>Derde-partijapps of wearables (Oura, enz.)</li>
          <li>Indirecte schade of gevolgschade</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          7.3 Aansprakelijkheidslimiet
        </h3>
        <p>
          Onze totale aansprakelijkheid beperkt zich tot het bedrag dat u in het voorgaande jaar aan YouCaps hebt betaald (of €50 als dit minder is).
        </p>

        <p style={{ marginTop: 'var(--space-md)', color: 'var(--ink-2)' }}>
          <em>
            Deze beperking is niet van toepassing op aansprakelijkheid die niet kan worden beperkt onder Nederlands/EU-recht
            (bijv. persoonlijk letsel, dood, opzet, grove schuld).
          </em>
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>8. Betaaldiensten & Abonnementen</h2>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          8.1 Gratis App
        </h3>
        <p>
          De volledige Ovari-app is <strong>GRATIS</strong>. Geen paywall, geen verborgen kosten.
        </p>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          8.2 Optionele Supplementen (Toekomst)
        </h3>
        <p>
          We zullen optioneel een Supplements abonnement aanbieden (€29/maand):
        </p>
        <ul>
          <li>U hebt <strong>GEEN</strong> abonnement nodig om de app te gebruiken</li>
          <li>U kunt het abonnement op elk moment annuleren</li>
          <li>30 dagen volledige geld-terug-garantie</li>
        </ul>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          8.3 Betaling
        </h3>
        <p>
          Betalingen worden verwerkt via Mollie (PCI-DSS compliant). Wij slaan creditcardgegevens <strong>NIET</strong> op.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>9. Beëindiging</h2>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          9.1 Uw Beëindiging
        </h3>
        <p>
          U kunt uw account op elk moment verwijderen via Instellingen → "Mijn gegevens verwijderen".
        </p>

        <h3 style={{ fontSize: '15px', fontFamily: 'var(--font-sans)', fontWeight: '600', marginBottom: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
          9.2 Onze Beëindiging
        </h3>
        <p>
          We kunnen uw account beëindigen als:
        </p>
        <ul>
          <li>U deze Voorwaarden schendt (bijv. illegale activiteiten)</li>
          <li>U aanhoudend misbruik maakt van de service</li>
          <li>Juridische verplichting (rechterlijk bevel)</li>
        </ul>

        <p style={{ marginTop: 'var(--space-md)' }}>
          We zullen u 30 dagen vooraf waarschuwen tenzij onmiddellijk handelen vereist is.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>10. Wijzigingen aan Deze Voorwaarden</h2>

        <p>
          We kunnen deze Voorwaarden bijwerken. Grote wijzigingen worden aangekondigd via e-mail en in-app.
          Uw voortgezette gebruik betekent acceptatie.
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>11. Geschillen & Rechtsmacht</h2>

        <p>
          Deze Voorwaarden worden beheerst door <strong>Nederlands recht</strong>.
        </p>

        <p style={{ marginTop: 'var(--space-md)' }}>
          <strong>Geschillenbeslechting:</strong>
        </p>
        <ol>
          <li>Neem eerst contact met ons op via info@youcaps.app, de meeste klachten lossen we onderling op</li>
          <li>Komen we er samen niet uit, dan kan het geschil worden voorgelegd aan de bevoegde Nederlandse rechter</li>
        </ol>
        <p style={{ marginTop: 'var(--space-md)', color: 'var(--ink-2)' }}>
          <em>
            Als consument behoudt u altijd de bescherming van het dwingende consumentenrecht van uw woonland.
          </em>
        </p>
      </section>

      <section style={{ marginBottom: 'var(--space-lg)' }}>
        <h2 style={{ fontSize: '20px', fontWeight: '500', marginBottom: 'var(--space-md)' }}>12. Contact</h2>

        <p>
          <strong>Voor vragen of klachten:</strong>
        </p>
        <p style={{ marginTop: 'var(--space-sm)' }}>
          YouCaps (eenmanszaak van Anneke Bakelaar)<br />
          Pietersonstraat 30<br />
          2684 XS Ter Heijde<br />
          KvK-nummer: 95822623<br />
          Btw-identificatienummer: NL005175416B50<br />
          E-mail: info@youcaps.app
        </p>
      </section>

      <p style={{ marginTop: 'var(--space-xxl)', borderTop: '1px solid var(--border)', paddingTop: 'var(--space-lg)', color: 'var(--ink-3)', fontSize: '13px', fontFamily: 'var(--font-sans)', fontWeight: '400' }}>
        <strong>Laatst bijgewerkt:</strong> 4 juni 2026<br />
        <strong>Versie:</strong> 1.0
      </p>
    </div>
  )
}
