    ...
    <div style={{ padding: '0 var(--space-lg)', marginBottom: 'var(--space-lg)', opacity: 0.4 }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-sm)', marginTop: 'var(--space-md)' }}>
        {[
          ...
          <div key={i} style={{ flex: 1, background: 'var(--d-card)', padding: 'var(--space-sm)', borderRadius: '2px', textAlign: 'center', border: '1px solid var(--d-border)' }}>
            <div style={{ fontSize: 'var(--font-size-micro)', color: 'var(--d-ink-3)', margin: '0 0 4px 0', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px', textShadow: '0 1px 3px rgba(0, 0, 0, 0.3)' }}>
              {getReadinessIcon(item.type)}
              <span>{item.label}</span>
            </div>
            <p style={{ fontSize: '18px', fontWeight: 'var(--font-weight-medium)', color: 'var(--d-ink)', margin: 0, textShadow: '0 1px 3px rgba(0, 0, 0, 0.3)' }}>
              {item.value}
            </p>
          </div>
        ))}
